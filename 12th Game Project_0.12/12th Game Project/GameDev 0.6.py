...
while True:
    ...
    flybarfraction = ((time.time() - prevflytime)/flydivconst)/maxflytime
    if flybarfraction > 1:
        flybarfraction = 1
    ...
    greenwidth = int((flwidth - 2) * flybarfraction)
    pygame.draw.polygon(scr, GREEN, ((fl_x + 2, fl_y + 1),
            (fl_x + greenwidth, fl_y), (fl_x + greenwidth, fl_y + flheight - 1),
    ...
    if flyflag and time.time() - startflytime > flybar:
        flyflag = False
        dy = -2
        prevflytime = time.time()
