...
flytime = 3   # Fly Key is f
flybar = 0      # Show how much fly you have left
...
spUpimg = pygame.image.load(supermanimg).convert() # Ak - Move Up
spDownimg = pygame.image.load(supermanimg).convert() # Ak - Move Down
    ...
    if dy != 2:
        ...
        for i in Plat.plist:
            ...
            if i.rect.x-skwidth-6 <= superman.rect.x <= (i.rect.x+pwidth+6) and -3<i.rect.y -(superman.rect.y+skheight)<3 :
                ...
                superman.image = spimg  # Ak - To change images while moving straight, moving up and moving down 
        ...
        if event.type == KEYDOWN:
            if event.key == K_SPACE:
                if (dy == 0 or fsj) and jump < 2: 
                    ...
                    fsj = not(fsj)
                    #superman.image = pygame.transform.rotate(superman.image, angle)   # Ak - AntiClockwise
                    superman.image = spUpimg  # Ak - Moving Up
            ...
            elif event.key in (K_f,):    # I thought (K_f, K_F)
                dy += 3
                flyflag = True
