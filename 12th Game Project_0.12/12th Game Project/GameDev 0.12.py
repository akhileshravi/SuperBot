...
water = pygame.image.load(waterimg).convert_alpha()
...
def quitter(score):
    ...
    scr.blit(font2.render('YOU DIED!', True, (255,0,0)), (scrwidth//2.5, int(scrheight/2.5)))
    while True:
...
def pixelator():
    ...
    notyet = []
    for i in xrange(0,scrwidth,10):
        for j in xrange(0,scrheight,10):
            notyet.append([i,j])
    sp = ' '
    while notyet:
        for i in xrange(6):
            ...
            a, b = j[0], j[1]
            pygame.draw.polygon(scr, (random.randint(0,255),random.randint(0,255),random.randint(0,255)), ((a,b),
            (a + 10, b), (a + 10, b + 10),
            (a, b + 10)))
        pygame.display.update()
        sleep(0.002)
...
xwater = 0
ywater = scrheight+50  #50 pixels below bottom of screen
minywater = 290  # water rises to this height
...
while True:
    ...
    if y_coord < -150:
        superman.rect.y += 5
        if superman.rect.y > (scrheight+30):
